...
- No readiness check for `show_last_workout` retrieval-only requests.
- No readiness check for plain plan display (`show_plan`, weekly overview).
...
## Workout-log structure (recommended)
- session: `startedAt`, `modality`, optional `durationMin`, `sessionNotes`
- exercise entry: `exerciseId`, `primary`, `secondary`, `intensity`
- gym sets: `weightKg`, `reps`, optional `rpe`, `rir`
- callanetics fields: `holdSec`, `pulses`, `controlQuality`, `restSec`
