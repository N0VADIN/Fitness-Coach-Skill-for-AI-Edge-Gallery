(function (globalScope) {
  ...
  function readinessDecision(score) {
    if (score >= 8) return 'full_intensity';
    if (score >= 6) return 'normal_training';
    if (score >= 4) return 'reduce_20_percent';
    return 'active_recovery_or_rest';
  }
  ...
  function computeReadiness(payload) {
    ...
    return {
      ...
      decision: readinessDecision(effectiveScore),
      stressedGroups,
      totalPenalty: Number(totalPenalty.toFixed(2)),
      global: { status: globalStatus, score: globalScore, decision: readinessDecision(globalScore), totalPenalty: Number(totalPenalty.toFixed(2)) },
      targeted: targetGroups.length > 0
        ? { targetGroups, status: targetedStatus, score: targetedScore, decision: readinessDecision(targetedScore), totalPenalty: targetedPenalty }
        : null
    };
  }
  ...
  function startWorkout(payload) {
    ...
    let message = `Starte ${planned.name}.`;
  ...
  function showLastWorkout(payload) {
    ...
    if (exerciseId) {
      last = [...sessions]
        ...
        .find((session) => (session.entries || []).some((e) => e.exerciseId === exerciseId)) || null;
    } else {
      last = getLastSession(sessions);
    }

    if (!last) {
      return { result: { action: 'show_last_workout', message: 'Noch kein passendes Workout geloggt.' } };
    }

    const entryCount = Array.isArray(last.entries) ? last.entries.length : 0;
    const base = `Letztes Workout: ${last.modality || 'training'} am ${last.startedAt} mit ${entryCount} Übungen.`;
    const msg = exerciseId ? `${base} Enthielt: ${exerciseId}.` : base;
  ...
  function targetedReadiness(payload) {
    ...
    return {
      result: {
        ...
        message: readiness.status === 'ready'
          ? 'Für die Zielbereiche bist du heute bereit.'
          : 'Für die Zielbereiche heute besser etwas vorsichtiger.'
      }
    };
  }

  function suggestAdjustment(payload) {
    const readiness = computeReadiness(payload);
    const suggestion = suggestIntensity(readiness, payload);
    return {
      result: {
  ...
  function weeklySnapshot(payload) {
    const sessions = Array.isArray(payload.sessions) ? payload.sessions : [];
    const weekStart = payload.weekStart ? parseIso(payload.weekStart) : null;
    const weekEnd = payload.weekEnd ? parseIso(payload.weekEnd) : null;
    ...
    const modalities = inWindow.reduce((acc, s) => {
      const key = s.modality || 'unknown';
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
    ...
    for (const session of inWindow) {
      for (const e of session.entries || []) {
        for (const g of [...(e.primary || []), ...(e.secondary || [])]) {
          groupCount[g] = (groupCount[g] || 0) + 1;
        }
      }
    }

    const mostLoadedGroups = Object.entries(groupCount)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([g]) => g);

    const readiness = computeReadiness({ ...payload, sessions: inWindow });
    ...
    return {
      result: {
        ...
        mostLoadedGroups,
        readiness,
        note: readiness.status === 'ready'
          ? 'Week looks balanced. Keep progressing.'
          : 'Week was demanding. Consider one lighter session.'
      }
    };
  }

  function microProgressionSuggestion(payload) {
    const modality = payload.plannedModality || 'gym';
    const readiness = computeReadiness(payload);
    const last = getLastSession(payload.sessions || []);
    ...
    if (modality === 'callanetics') {
      rules = [
        'Increase hold time by 5-10 sec if control quality stays high.',
        'Add 5-10 pulses per set if form stays stable.',
        'Or reduce rest by 10-20 sec if caution is not present.'
      ];
    } else {
      rules = [
        'If top rep range reached with <=2 RIR, add 2.5-5 kg.',
        'If load unchanged, add 1-2 reps total next session.',
        'If caution/recover, keep load and reduce one set.'
      ];
    }
  ...
  function execute(payload) {
    ...
    if (action === 'start_workout') return startWorkout(payload);
    ...
    if (action === 'targeted_readiness') return targetedReadiness(payload);
    if (action === 'suggest_adjustment') return suggestAdjustment(payload);
    if (action === 'weekly_snapshot') return weeklySnapshot(payload);
    if (action === 'micro_progression_suggestion') return microProgressionSuggestion(payload);
  ...
  globalScope.FitnessCoachReadiness = {
    ...
    suggestAdjustment,
    weeklySnapshot,
    microProgressionSuggestion,
    // snake_case aliases for explicit session-coach action parity
    show_today: showToday,
    start_workout: startWorkout,
    log_workout: logWorkout,
    show_last_workout: showLastWorkout,
    suggest_next_workout: suggestNextWorkout,
    create_hybrid_plan: createHybridPlan,
    targeted_readiness: targetedReadiness,
    suggest_adjustment: suggestAdjustment,
    weekly_snapshot: weeklySnapshot,
    micro_progression_suggestion: microProgressionSuggestion
  };
